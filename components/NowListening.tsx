...
import { musicBar, musicBars, playKeyFrames, text, track } from "./NowListening.css";
...
export const NowListening: FC<NowListeningProps> = () => {
    ...
    return (
        <Stack gap="1" flexDirection="column" css={{ width: "fit-content" }}>
            {isLoading ? (
                <Text color="text" className={text}>
            ...
            <Fragment>
                <Box as="a" href={state?.track.externalUrl} target="_blank" className={track}>
                    {state?.track.name}
                </Box>
                <Text color="text">{state?.track.artists.map((a) => a.name).join(", ")}</Text>
            </Fragment>
        </Stack>
    );
};
